...
export default class AuthService {
  ...
  async lostPassword({ email }) {
    const url = this.urlBuilder.createUrl("lost_password");
    const body = {
      email,
      client_id: this.client.clientId,
    };
  ...
  async resetPassword({ newPassword, email, resetToken }) {
    let url = this.urlBuilder.createUrl("reset_password");
    url += `?access_token=${resetToken}`;
    const body = {
